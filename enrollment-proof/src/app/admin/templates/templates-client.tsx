...
export default function TemplatesClient() {
  ...
  // ✅ canonical text body (what you already had)
  const [body, setBody] = useState("");

  // ✅ new: stored HTML body (for rich formatting later)
  ...
  function startEdit(t: TemplateRow) {
    ...
    // For body, we prioritize the new HTML version if it exists, but fall back to the old plain text for legacy templates
    const text = (t.body ?? "") as string;
    setBody(text);

    // HTML (optional)
    setBodyHtml(String(t.body_html ?? ""));
  ...
  // plain text fallback (legacy + canonical)
  body: body.trim(),

  // rich html (can be empty)
...
HR Team`;
                  setBody(t);
                  setBodyHtml(""); // will be generated by editor callback as they type
                }}
